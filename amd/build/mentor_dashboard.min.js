...
function(Ajax, Notification, ModalFactory, ModalEvents, Str) {
    ...
    const handleAddMenteeSubmit = (modal, selectRequired) => {
        ...
        if (!menteeid || menteeid <= 0) {
            ...
            if (errDiv) { errDiv.textContent = selectRequired; errDiv.classList.remove('d-none'); }
            return;
        }
        ...
        Ajax.call([{
            ...
            if (result.success) {
                ...
                if (errDiv) { errDiv.textContent = result.message; errDiv.classList.remove('d-none'); }
            }
        }).fail(function(ex) {
            modal.hide();
            Notification.exception(ex);
        ...
        dashboardRoot.addEventListener('click', function(e) {
            ...
            Str.get_strings([
                ...
                return ModalFactory.create({
                    ...
                    title: strs[0],
                    body:  buildAddMenteeForm(strs[1]),
                }).then(function(modal) {
                    modal.getRoot().on(ModalEvents.save, function() {
                        handleAddMenteeSubmit(modal, strs[3]);
                    });
                    modal.show();
                    setupMenteeSearch(modal, strs[2]);
                    return modal;
                });
            }).catch(Notification.exception);
        });
